...
row_code <- ggplot() + 
 ...
 row_total=n())", 
           size=1.8, hjust="left", vjust="top", family="Iosevka") +
  scale_x_continuous(expand=c(0,0), limits=c(0,1.5)) +
  scale_y_continuous(expand=c(0,0), limits=c(.9,1.5)) +
  theme_nothing()
...
plot <- 
  plot_grid(plot_imd_driver,spacer,  
            ...
            rel_widths = c(1,1, 1,.05, .75), 
            rel_heights = c(1,1,1,1,1),
            nrow=2) + theme_map()


code <- exp_code / row_code

plot <- 
  plot_grid(plot_imd_driver, plot_imd_driver_resid, plot_cols, code,  
            obs_code, resid_code, spacer,
            rel_widths = c(1, 1, .75, .7), 
            rel_heights = c(1,.5),
            nrow=2) + theme_map()
