...
## Skills outcomes
  - [ ] ggplot2 code parametered with encoding and layouts to support comparison.
  - [ ] `tidyverse`-style code for computing model-expected values.
:::
...
The early stages of an EDA may be very data-driven. Datasets are described abstractly according to their measurement level and corresponding data graphics and summary statistics generated from these descriptions (e.g. @tbl-variable-types-explore). As knowledge and understanding of the dataset increases, researchers might apply more targeted theory and prior knowledge when developing and evaluating models.
...
Now we've identified the data for our analysis, we can begin to explore the dataset, bearing in mind the ultimate -- who-hit-whom -- question: Do drivers share demographic characteristics with the pedestrians they crash into, and does this vary by the location in which crashes take place? 
...
The modelling is somewhat involved -- a more gentle introduction to model-based visual analysis appears in Chapters [-@sec-model] and [-@sec-uncertainty] -- but code for generating the model and graphics in @fig-imd-driver-cas-crash can be exposed by clicking the dropdown symbol below.
