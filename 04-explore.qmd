...
The early stages of an EDA may be very data-driven. Datasets are described abstractly according to their measurement level and corresponding data graphics and summary statistics suggested from this (e.g. @tbl-variable-types). As knowledge and understanding of the dataset increases, researchers might apply more targeted theory and prior knowledge when developing and evaluating models.
...
An [`.fst`](https://www.fstpackage.org/) dataset containing pedestrian crashes linked to casualty and driver characteristics has been stored in the book's accompanying [data repository](https://github.com/vis4sds/data). There are simplifications and assumptions made when generating this linked dataset, detailed in the template file.
...
<!-- The focus of our analysis is in inequalities in the characteristics of those involved in pedestrian crashes. For this, the IMD status of the pedestrian, driver and crash location must be recorded. -->
...
2. *Encoding*: Bars arranged vertically (`y` position) on frequency and horizontally (`x` position) on `imd` class.
