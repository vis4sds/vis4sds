...
To investigate this more directly it makes sense to *position* nodes (boroughs) with a geographic arrangement.  In the right of @fig-lines-geog, boroughs are placed in their exact geographic position (geometric centroid of boroughs) and line width and colour is used to encode edge (commuter flow) frequency. Boroughs are again represented with circles sized according to frequency (the total number of jobs and workers contained in the borough) and commuter flow *direction* is encoded by making lines asymmetric, following @wood_visualizing_2011: the straight ends are origins, the curved ends destinations. \index{maps!flowmaps} \index{maps!bezier flowlines}
...
Once a gridmap polygon file (`grid`) and corresponding cell positions (`row` and `col`) is generated, gridmaps can be plotted from the polygon file directly, as in plot *(a)* of @fig-map-code, by supplying grid square positions to `facet_grid()`, plot *(b)*, or combining both to effect a map-within-map layout for OD maps, plot *(c)*.
...
6. *Setting*: `coord_flip()` for bars that are oriented horizontally.
 ...
 Adding a geospatial arrangement, as in @fig-nodes-gridmap, can further help with exploring the geography to these different categories of borough. More balanced boroughs to the east (Barking and Dagenham BaD) and west (Hillingdon Hil); worker-rich boroughs (left-pointing bars) with large proportions of professional workers in west and south west London (Wandsworth Wnd, Richmond Upon Thames RuT); job-rich boroughs (right-pointing bars) in central London (Westminster Wst, Camden Cmd).
