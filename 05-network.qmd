...
The `od_pairs` dataset is displayed in @tbl-census-ods. Each observation is a unique OD pair summarising the total number of recorded commuters between a pair of London boroughs for a stated occupation type.
...
**TO DO: Include a figure with a simplified explanation of two ways in which gridmaps can be plotted -- on `geom_sf()`, on `facet_grid()` on both `map-within-wap`. Also imroved gridmap solution (Cmd poor).**
