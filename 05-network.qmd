...
- [x] Undestand the special structure and vocabulary used to represent network data.
...
The left graphic in @fig-lines-geog uses a force-directed layout to represent the travel-to-work data. Nodes, London boroughs, are sized according to the number of jobs and workers they contain and edges, commutes between boroughs, are represented as lines sized by commute frequency. As is often the case with node-link diagrams, the graphic looks complex. Job-rich borouhgs, Westminster and City of London, are labelled and have many connecting lines -- most likely  workers commuting-in from other London boroughs. Other more 'residential' boroughs (Lambeth and Wandsworth) are labelled. These two contain many connecting lines --  likely residents commuting-out to other London boroughs  most likely  workers commuting-in from other London borough for work. That those labelled boroughs in force-directed space are also close to one-another in geographic space suggests that, as expected, between-borough commuting has a spatially dependent element.
...
#| out.width: 100%
#| fig-cap: "Flowlines with edges showing frequencies between London boroughs. **TODO: annotate other London boroughs.**"
#| echo: false
#| eval: true
#| warning: false
...
#| label: fig-matrices
#| out.width: 100%
#| fig-cap: "Origin-destination matrices ordered according to borough size on number of jobs."
#| echo: false
#| eval: true
#| warning: false
...
## Task
Although OD maps overcome several problems of flow-line based visualizations,  share several of the characteristics of effective data graphics discussed in Chapter [-@sec-visual], they do nevertheless take some interpretation, especially when seen for the first time.
...
trajectories <- lon_geogs |>
  ...
  mutate(
    trajectory = map(
      data,
      ~get_trajectory(
        .x$x_real, .x$y_real, .x$x_grid, .x$y_grid, .x$ward_name
        )
    )
  ) |>
  select(trajectory) |>
...
#| out.width: 100%
#| fig-cap: "Workers and jobs in London borough by occupation class. Bars counts are locally-scaled by borough.  **TODO: remove img caption.**"
#| echo: false
#| eval: true
#| warning: false
...
6. *Setting*: `coord_flip()` for bars that are oriented horizontally.
 ...
 Adding a geospatial arrangement -- @fig-nodes-gridmap -- is instructive at exposing the geography to these different categories of borough. More balanced boroughs to the east (Barking and Dagenham BaD) and west (Hillingdon Hil); worker-rich boroughs (left-pointing bars) with large proportions of professional workers in west and south west London boroughs (Wandsworth Wnd, Richmond Upon Thames RuT); job-rich boroughs (right-pointing bars) in central London (Westminster Wst, Camden Cmd).
...
### Analysing over the edges (flows between boroughs)
