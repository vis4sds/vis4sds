...
- [ ] Write `ggplot2` specifications to analyse geospatial network origin-destination data.
...
To investigate this more directly it makes sense to *position* nodes (boroughs) with a geographic arrangement.  In the right column of @fig-lines-geog, boroughs are placed in their exact geographic position (geometric centroid of boroughs) and line width and colour is used to encode edge (commuter flow) frequency. Boroughs are again represented with circles sized according to frequency (the total number of jobs and workers contained in the borough) and commuter flow *direction* is encoded by making lines asymmetric, following @wood_visualizing_2011: the straight ends are origins, the curved ends destinations.
...
#| label: fig-matrices
#| out.width: 100%
#| fig-cap: "Origin-destination matrices ordered according to borough size on number of jobs. A separate, local, colour scale is created for each destination borough."
#| echo: false
#| eval: true
#| warning: false
...
The `od_pairs` dataset is displayed in @tbl-census-ods, and is effectively an *edges* table. Each observation is a unique OD pair summarising the total number of recorded commuters between a pair of London boroughs for a stated occupation type.
...
The main function to call is `points_to_grid()`.  This takes a tibble of geographic points and returns grid cell positions (*row* and *column* identifiers). In the code below an 8x8 grid is used. The allocation is also constrained by a *compactness* parameter which determines the extent to which points are allocated to cells in the centre (compactness = 1), edges (0) or scaled geographic location (0.5) within the grid.
...
**TO DO: Include a figure with a simplified explanation of two ways in which gridmaps can be plotted -- on `geom_sf()`, on `facet_grid()` on both `map-within-wap`**
...
* Scale: `scale_fill_distiller()` for a diverging colour scheme using the ColorBrewer [@harrower_colorbrewerorg_2003] `RdBu` palette and made symmetrical on 0 by manually setting `limits()`.
...
Not on network visualization per se, but presents a set of data graphics on London. Worth highlihgting here is the use of annotations and descriptions to efficiently explain the data graphcs, a theme we return to in @sec-storytelling:
