...
#| out.width: 100%
#| fig-cap: "Bayesian Risk Ratios comparing pedestrian injury severity rates in English local authorities coloured according to 'statistical significance', whether the bootstrap confidence interval does not cross 1.0. In the right map, a random intercept is added on London. **TODO: Make marks bigger, make text annotations bigger, make grey lihgter**"
#| echo: false
#| eval: true
...
The technical element demonstrates how some of the uncertainty estimates in the chapter can be reproduced.  We will again make use of functional programming approaches via the [`purrr`](https://purrr.tidyverse.org/) package, most obviously for generating and working over bootstrap resamples.

### Import

* Download the [07-template.qmd]() file for this chapter and save it to the `reports` folder of your `vis4sds` project.
* Open your `vis4sds` project in RStudio and load the template file by clicking `File` > `Open File ...` > `reports/07-template.qmd`.
...
2. *Encoding*: x- position varies according to local authority and y-position according to KSI rate. The estimated KSI rate and bootstrap standard error is also passed to `stat_dist_gradientinterval()`, the `ggdist` function for producing gradient plots.
