...
Perhaps the most high-profile example of data graphics specialised to communication were those produced by the \index{Financial Times} Financial Times [@ft_coronavirus_2020]. @fig-ft-vis displays one such example, the Financial Times's Covid-19 trajectory tracker. Along the y-axis are cumulative numbers of deaths represented using a log-scale; and along the x-axis, the number of days that elapsed since some threshold number of deaths was recorded.
...
We can evaluate the graphic using some of the principles introduced in @sec-visual. In its use of position on an aligned scale to encode death counts, colour hue to differentiate countries and lines to connect daily counts, the graphic's visual encoding exploits our cognitive abilities. The graphic also makes appropriate use of \index{exploratory data analysis!comparison strategies} superimposition to support comparison [@gleicher_visual_2011] -- lines for each country represented on the same coordinate space.


More interesting is how the graphic performs on @roth_cartographic_2020's characteristics of visual storytelling. It is clear that the graphic is *designed* with a very deliberate purpose:
