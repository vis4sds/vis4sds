...
1. We start with a data frame, in this case each observation is an electoral result for a Parliamentary Constituency. This is passed to the ggplot2 spec using the pipe operator (`%>%`). We also identify the variables to encode and their measurement <span style="color:#5A91CA;font-weight:bold">type</span>. Remembering the previous chapter, both `con_1719` and `leave_hanretty` are `ratio` scale variables.
...
[**RB: remove section on perceptual rankings, detection and estimation?**]
