...
Dot-density maps also convey absolute numbers of votes but in a way that preserves geography. In the example below, each dot represents 1,000 votes for a given party -- Conservative, Labour, Other -- and dots are positioned in the constituencies from which those votes were made. Dots therefore concentrate in population-dense areas of the country. The difficulty in generating dot-density maps is not in wrangling ggplot2, but in preparing data to be plotted. We need to create a randomly located point within a constituency's boundary for every thousand of votes that are made there.  R packages speciased to dot-density maps provide functions for doing  this, it is quite easy to achieve using the sorts of functional and `tidyverse`-style code introduced throughout this book. 
