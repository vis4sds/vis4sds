...
- [x] Load flat file datasets in RStudio.
...
In @wickham_r_2017, the benefits of tidy layouts are demonstrated with the [`gapminder`](https://www.gapminder.org/data/) dataset and with reference to key data processing functions in R.  To consolidate our understanding of *tidy* data let's quickly look at the `gapminder` data, as it is an example that we're probably more likely to encounter in SDS research.
...
kbl(gapminder_untidy2)
```
From the a*tidy* version of the data, we can identify the *variables* and note that each *observation* is a recorded count of cases and population for a country in a year.
...
The technical element to this chapter involves importing, describing, transforming and tidying data from a large bikeshare scheme -- New York's [Citibike](https://www.citibikenyc.com/) scheme.
...
fst::write_csv(stations, here("data", "bikedata", "ny_stations.csv"))
...
The object type of a variable in a data frame relates to that variable's *measurement level*. It is often useful to convert to types with greater specificity. For example, we may which to convert the `start_time` and `stop_time` variables to a date-time format so that various time-related functions can be used. For efficient storage, we may wish to convert the *station identifier* variables as `int` types by removing the redundant "ny" text which prefaces `end_station_id`, `end_station_id`, `stn_id`. The geographic coordinates are currently stored as type `chr`. These could be regarded as quantitative variables, floating points with decimals. So converting to type `dbl` or as a [`POINT`](https://r-spatial.github.io/sf/articles/sf1.html#simple-feature-geometry-types) geometry type (more on this later in the course) may be sensible.
...
ny_trips |>  # Take the ny_trips data frame.
  count(user_type, sort=TRUE) # Run the count function over the data frame and set the sort parameter to TRUE.
##   user_type       n
...
kbl(aggregate_functions)
```

Often you will want to do more than simply counting and you may also want to be more explicit in the way the data frame is *grouped* for computation. We'll demonstrate this here with a more involved analysis of the usage data and using some key aggregate functions (@tbl-aggregate-functions).
...
#| eval: false
ny_trips |> # Take the ny_trips data frame.
  group_by(user_type) %>% # Group by user_type.
...
::: {.callout-note}
Remembering that *pipes* (`|>`) take the output of a set of code and make it the input of the next set, you will notice t separate lines for each call to the *pipe* operator. This is good practice for supporting readability of your code.
:::
...
ny_temporal <- ny_trips |>  # Take the ny_trips data frame.
  ...
  group_by(user_type, day, hour) %>% # Group by day, hour, user_type.
...
Developing the vocabulary and technical skills to systematically describe and organise data is crucial to modern data analysis. This chapter has covered the fundamentals here: that data consist of *observations* and *variables* of different types [@stevens_on_1946] and that in order to work effectively with datasets, especially in a functional way in R, these data must be organised according to the rules of *tidy*  data [@wickham_tidy_2014]. Most of the content was dedicated to the techniques that enable these concepts to be operationalised. We covered how to download, transform and reshape a reasonably large set of data from New York's [Citibike]() scheme.  In doing so, we generated insights that might inform further data collection and analysis activity. In the next chapter we will apply and extend this conceptual and technical knowledge as we introduce the fundamentals of visual data analysis and `ggplot2`'s grammar of graphics.
