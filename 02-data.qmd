...
An alternative organisation is in @tbl-gapminder (b). This is *untidy* as the observations are spread across two rows, makiing operations that we might want to perform on the `cases` and `population` variables -- for example computing exposure rates -- somewhat tedious.
...
First we'll count the number of trips made in Jun 2020 by gender. `dplyr` has a convenience  function for counting, so we could run the code below, also in the [02-template.Rmd]() for this session. The code block is commented to convey what each line achieves.
