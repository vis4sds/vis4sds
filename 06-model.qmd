...
- [ ] Write ggplot2 code to generate graphics for exploring multivariate association and presenting regression outputs (faceted scatteplots, parallel coordinate plots, dot plots with error bars).
...
vars <- tibble::tibble(
  ...
  kbl(vars, protect_latex = TRUE, escape=FALSE) |>  kable_styling(font_size = 9) |>
      pack_rows("post-industrial / knowledge economy", 1, 4, bold=FALSE, label_row_css = "border-bottom: 0px solid;") |>
      pack_rows("diversity/values/outcomes", 5, 8, bold=FALSE, label_row_css = "border-bottom: 0px solid;") |>
      pack_rows("metropolitan / 'big city'", 9, 10, bold=FALSE, label_row_css = "border-bottom: 0px solid;") |>
     # column_spec(1, width = "60%") |>
...
#| out.width: 100%
#| fig-cap: "Output from multiple regression model of Leave vote by demographic composition of constituency with a Fixed Effect and interaction term on region."
#| echo: false
#| eval: true
#| warning: false
...
3. *Generate outputs*: Next, the different cuts of model outputs can be mde using `glance(), tidy(), augment()`, with `map()` to iterate over the list of model objects. The new columns are now `list-columns` of data frames containing model outputs.
