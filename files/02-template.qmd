...
The [`fst`](https://www.fstpackage.org/) package is then used for serializing and reading in the these data. `fst` implements in the background various operations such as multi-threading to work on large datasets in-memory in R. Data are then written out to disk with `.fst` and read back into R. This code block takes some time to run and there is also a problem with how `bikedata` handles the New York record data. The code below is therefore for demonstration purposes should you wish to work with other bikeshare data. If you wish, jump to the next code block where we download a cleaned version of the New York data from from an external repository.
