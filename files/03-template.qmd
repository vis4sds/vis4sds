...
Reopen your project and save this `.qmd` to the `reports` folder. When working through the materials from this session run each code chunk separately by clicking the small green arrow beside each code chunk. By default we have set `eval : false` on most code chunks, so when you knit the code chunk will not execute.
